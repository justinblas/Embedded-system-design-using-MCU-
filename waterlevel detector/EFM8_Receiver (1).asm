...
Wait_one_second:	
 ;For a 72MHz clock one machine cycle takes 1/72MHz=13.89 ns

    mov R2, #200 ; Calibrate using this number to account for overhead delays
X3: mov R1, #250
X2: mov R0, #240
X1: nop
	nop
	nop
	djnz R0, X1 ; 1+1+1+3 machine cycles -> 6*13.89 ns*240=20us (see table 10.2 in reference manual)
    djnz R1, X2 ; 20.0016us*250=5ms
    djnz R2, X3 ; 5ms*20=0.1s 
...
;--------------------------------------------------------------;
; Dumps the 6-digit packed BCD number in R2-R1-R0 into the LCD ;
;--------------------------------------------------------------;
...
Init_L2:
    ...
    orl P0SKIP, #0b_0000_1000 ; P0.7 and P0.6 used by LCD.  P0.3 used as CS/ for SPI memory.
	orl P1SKIP, #0b_0000_0011 ; P1.0 and P1.1 used by LCD
