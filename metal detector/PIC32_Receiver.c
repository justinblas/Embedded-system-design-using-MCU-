...
/*static const unsigned long int wav_index[5]={
    0x00002d, // 0 
    0x004d0f, // 1 
...
static const unsigned long int wav_bytes[5]={
 19682,
 21413,
 36392,
 21755,
...
float get_temperature(int adcval)
{
	float voltage;
	float temperature;
...
int main(void)
{
	...
	// flag
	 int small_ferrous,small_non_ferrous, large_ferrous, large_non_ferrous, no_metal = 0;
   
    unsigned long int wav_index[]={ 45,19727,41140, 77532, 99287 };
     unsigned long int wav_bytes[]={ 19682,21413,36392,21755, 39770 };
	DDPCON = 0;
	CFGCON = 0;
  ...
  // period 
	...
	ANSELBbits.ANSB3 = 1;    // setRB3(AN5, pin 7 of DIP28) as anlog input 
	TRISBbits.TRISB3 = 1;    // set RB3 as an input 

	TRISBbits.TRISB6 = 0;    // sey RB6 as output 
	LATBbits.LATB6 = 0;      // set output to 0 ;
	 ...
	 f = get_frequency();
		if(f >= 21900.0){
		   large_non_ferrous = 1;
		   	...
		   	printf("large non ferrous detect  ");
		   		Start_Playback(wav_index[4],wav_bytes[4]);	   
			T1CONbits.ON = 1;  
			while(play_flag !=0);
		
		  }
		else if(f < 21900.0 && f >21520){
		   large_ferrous = 1;
		   
		   	 
		   printf("large ferrous detect ");
		   Start_Playback(wav_index[3],wav_bytes[3]);	   
			T1CONbits.ON = 1;  
			while(play_flag !=0);
		   }
		else if(f <= 21519 && f > 21380){
		   small_non_ferrous =1;
		   
		   printf("small non ferrous detect ");
		    Start_Playback(wav_index[2],wav_bytes[2]);	   
			T1CONbits.ON = 1; 
			while(play_flag !=0);
		   }
		else if(f <= 21379 && f > 21359){
		   small_ferrous = 1;
		    
		   printf("small  ferrous detect ");
		   Start_Playback(wav_index[1],wav_bytes[1]);	   
			T1CONbits.ON = 1; 
			while(play_flag !=0);
		   }
		 else{
		   no_metal = 1;
		    ...
		    printf("no metal detect  ");
		    	Start_Playback(wav_index[0],wav_bytes[0]);	   
			T1CONbits.ON = 1; 
			while(play_flag !=0);
	...
	/*	if(no_metal){
		...
		else if(small_ferrous){
			Start_Playback(wav_index[1],wav_bytes[1]);	   
			T1CONbits.ON = 1;  
			while(play_flag !=0);
		}
		else if(small_non_ferrous){
		 Start_Playback(wav_index[2],wav_bytes[2]);	   
			T1CONbits.ON = 1; 
			while(play_flag !=0);
		}
		else if(large_ferrous){
		 Start_Playback(wav_index[3],wav_bytes[3]);	   
			T1CONbits.ON = 1; 
			while(play_flag !=0);
		}
		else if(large_non_ferrous){
		Start_Playback(wav_index[4],wav_bytes[4]);	   
			T1CONbits.ON = 1; 
			while(play_flag !=0);
		}
